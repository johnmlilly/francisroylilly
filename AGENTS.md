...
- Dev server: `npm run dev` (http://localhost:4321, runs in workerd via the
  Cloudflare Vite plugin). The user runs this themselves; do not start it.
- Build: `npm run build`
...
- DB: `npm run db:migrate` (apply Drizzle migrations), `npm run db:seed`
- Deploy: `npm run deploy` is a manual escape hatch only. Normal deploys run
  automatically via Cloudflare Workers Builds on push to `main`.

Data access is Drizzle ORM + `@libsql/client` against Turso (`db/client.ts`);
`TURSO_DATABASE_URL`/`TURSO_AUTH_TOKEN` are Worker secrets. Under Vitest,
`db/client.ts` uses an in-memory SQLite DB.

The user handles all commits and merges. Do not commit or merge without
...
Audit defaults to `gate=new-only`: only findings introduced by the current changeset affect the verdict. Inherited findings on touched files are reported under `attribution` and annotated with `introduced: false`, but do not block the commit. To gate every finding in changed files, set `"audit": { "gate": "all" }` in `.fallowrc.json`, which is the config this project loads. Do not add a `fallow.toml`: fallow loads one config file, and a new one would take precedence over `.fallowrc.json` and drop its reviewed exceptions.
