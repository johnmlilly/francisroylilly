...
**Build attempt:** 1
**Status:** in progress (partial work already on branch, see Notes)
...
- `SubscribeForm.astro` embedded in `Footer.astro` and `/blog`.
...
- Site-wide subscribe popup after ~9s with persisted dismissal.
...
- `src/components/SubscribeForm.astro` (new), `Footer.astro`, `src/pages/blog/index.astro`, `src/layouts/BaseLayout.astro`
...
- **Popup implementation.** Plan says `react-modal` + a second React form
  ...
  behavior (9s delay, Esc/backdrop/× close, persisted dismissal). Say
